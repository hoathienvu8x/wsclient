...
wsclient *libwsclient_new(const char *URI, int as_thread)
{
  ...
  if (as_thread)
  {
    update_wsclient_status(client, FLAG_CLIENT_IS_THREAD, 0);
  }

  if (TEST_FLAG(client, FLAG_CLIENT_IS_THREAD))
  {
    if (pthread_create(
...
void libwsclient_start_run(wsclient *c)
{
  ...
  {
    if (TEST_FLAG(c, FLAG_CLIENT_IS_THREAD))
    {
...
void libwsclient_close(wsclient *client, char *reason)
{
  ...
  if (!TEST_FLAG(client, FLAG_CLIENT_CLOSEING))
  {
    libwsclient_send_data(
      client, OP_CODE_CONTROL_CLOSE, (unsigned char*)reason,
...
)
{
  ...
  int i, frame_count = (payload_len + MAX_PAYLOAD_PAD) / MAX_PAYLOAD_PAD;
...
{
  libwsclient_send_data(
    client, OP_CODE_CONTROL_PING, (const unsigned char*)payload,
    payload ? strlen(payload) : 0
  );
}
